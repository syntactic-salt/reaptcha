...
test.serial(
  'should inject non-enterprise version of recaptcha by default',
  t => {
    t.plan(3);
    ...
    const script = document.scripts[0];
    t.truthy(script.src);
    t.regex(script.src, new RegExp('api.js'));
  }
);

test.serial('should inject enterprise version of recaptcha if specified', t => {
  ...
  mount(<Reaptcha {...defaultProps} enterprise={true} />);

  t.is(document.scripts.length, 1);
  const script = document.scripts[0];
  t.truthy(script.src);
  t.regex(script.src, new RegExp('enterprise.js'));
});

test.serial(
  'should execute enterprise version of recaptcha with action name',
  t => {
    window.grecaptcha.enterprise = {
      ready: callback => callback(),
      render: renderSpy,
    ...
    return wrapper
      ...
      .then(() => {
        t.true(executeSpy.calledOnce);
        t.true(
          executeSpy.calledWith(sinon.match.any, { action: 'SUPER_SAFE_PAGE' })
        );
      });
  }
);
